    ...
    for loader, stub in pattern_stubs:
        ...
        with open(filename, "r") as fh:
            ...
            for mobj in re.finditer(pattern_pat, pattern_data):
                method, string, start_angle, name = mobj.groups()
                registry[root_data["modid"] + ":" + name] = (
                    string, start_angle, method == "registerPerWorld")  # method == "registerPerWorld")
    return registry
...
def fetch_recipe(root_data, recipe):
    ...
    gen_resource_dir = root_data["resource_dir"].replace("/main/", "/generated/").replace("common/",
                                                                                          "forge/")  # TODO hack
...
def category_spoilered(root_info, category):
    if (not category["entries"]):
        return False  # fix for empty categories (like the "patterns" category if mod doesn't add any non-spell patterns) being spoilered
